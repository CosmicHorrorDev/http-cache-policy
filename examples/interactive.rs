...
fn make_a_request(cache: &mut Cache, cache_options: CacheOptions) {
    ...
    for (name, value) in resp.headers() {
        println!(
            "{}: {}",
            bold(name.as_str()).blue(),
            style(value.to_str().unwrap()).italic()
        );
    }
...
fn list_cache_entries(cache: &Cache) {
    ...
    for (uri, (policy, body)) in cache {
        let (stale_msg, ttl) = if policy.is_stale(current_time()) {
            (
                bold("stale").magenta(),
                style("ttl - expired".to_owned()).italic(),
            )
        } else {
...
use helpers::{bold, current_duration, current_m_ss, current_time, select_prompt, setup_req};
mod helpers {
    use std::time::{Duration, SystemTime};

    use super::{Req, CURRENT_TIME, START, THEME};
    ...
    pub fn select_prompt() -> Select<'static> {
        Select::with_theme(&*THEME).default(0)
    }
...
mod server {
    use super::{bold, current_duration, Req, Resp, CURRENT_TIME, START};
